    ...
    while True:
        ...
        for item in items:
            ...
            if not phone:
                ...
                try:
                    neg_detail = fetch_json(f'https://api.hh.ru/negotiations/{neg_id_tmp}', hh_headers)
                    for c in (neg_detail.get('resume') or {}).get('contact') or []:
                        if isinstance(c, dict) and c.get('type') == 'cell':
                            val = c.get('value')
                            phone = val.get('formatted', '') if isinstance(val, dict) else str(val or '')
                            break
...
def handler(event: dict, context) -> dict:
    """Загрузка откликов и вакансий работодателя с HH.ru + синхронизация в БД + вебхуки"""
    ...
    # POST — синхронизация коллекции в БД (бывший hh-sync)
    if method == 'POST':
        body = json.loads(event.get('body') or '{}')
